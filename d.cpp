...
const double cutoff = std::pow(2,1/6); 
const double modi_LJ= 24 * epsilon * (2*sigma12/ std::pow(cutoff,13) - sigma6 / std::pow(cutoff, 7));
...
DVector Force(const singleParticle &a, const singleParticle &b) {
    ...
    #ifdef __CUTOFF__
        if (fabs(R) < cutoff){
            LennardJonesForce = LennardJonesForce - r.e() * modi_LJ; 
        }else{
            LennardJonesForce.SetZero();
        }
